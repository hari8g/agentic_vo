...
import { IAgenticUpdateService } from '../../workbench/contrib/agentic/common/agenticUpdateService.js';
import { MetricsMainService } from '../../workbench/contrib/agentic/electron-main/metricsMainService.js';
import { AgenticMainUpdateService } from '../../workbench/contrib/agentic/electron-main/agenticUpdateMainService.js';
import { LLMMessageChannel } from '../../workbench/contrib/agentic/electron-main/sendLLMMessageChannel.js';
import { AgenticSCMService } from '../../workbench/contrib/agentic/electron-main/agenticSCMMainService.js';
import { IAgenticSCMService } from '../../workbench/contrib/agentic/common/agenticSCMTypes.js';
import { MCPChannel } from '../../workbench/contrib/agentic/electron-main/mcpChannel.js';
/**
...
export class CodeApplication extends Disposable {
	...
	private async initServices(machineId: string, sqmId: string, devDeviceId: string, sharedProcessReady: Promise<MessagePortClient>): Promise<IInstantiationService> {
		...
		// Agentic main process services (required for services with a channel for comm between browser and electron-main (node))
		services.set(IMetricsService, new SyncDescriptor(MetricsMainService, undefined, false));
		services.set(IAgenticUpdateService, new SyncDescriptor(AgenticMainUpdateService, undefined, false));
		services.set(IAgenticSCMService, new SyncDescriptor(AgenticSCMService, undefined, false));
	...
	private initChannels(accessor: ServicesAccessor, mainProcessElectronServer: ElectronIPCServer, sharedProcessClient: Promise<MessagePortClient>): void {
		...
		// Agentic - use loggerChannel as reference
		const metricsChannel = ProxyChannel.fromService(accessor.get(IMetricsService), disposables);
		mainProcessElectronServer.registerChannel('agentic-channel-metrics', metricsChannel);

		const agenticUpdatesChannel = ProxyChannel.fromService(accessor.get(IAgenticUpdateService), disposables);
		mainProcessElectronServer.registerChannel('agentic-channel-update', agenticUpdatesChannel);

		const sendLLMMessageChannel = new LLMMessageChannel(accessor.get(IMetricsService));
		mainProcessElectronServer.registerChannel('agentic-channel-llmMessage', sendLLMMessageChannel);

		// Agentic added this
		const agenticSCMChannel = ProxyChannel.fromService(accessor.get(IAgenticSCMService), disposables);
		mainProcessElectronServer.registerChannel('agentic-channel-scm', agenticSCMChannel);

		// Agentic added this
		const mcpChannel = new MCPChannel();
		mainProcessElectronServer.registerChannel('agentic-channel-mcp', mcpChannel);
