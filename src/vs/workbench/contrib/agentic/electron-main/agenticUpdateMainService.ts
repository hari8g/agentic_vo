...
export class AgenticMainUpdateService extends Disposable implements IAgenticUpdateService {
	...
	async check(explicit: boolean): Promise<AgenticCheckUpdateRespose> {
		...
		if (this._updateService.state.type === StateType.Ready) {
			// Update is ready
			return { message: 'Restart Agentic to update!', action: 'restart' } as const
		}
	...
	private async _manualCheckGHTagIfDisabled(explicit: boolean): Promise<AgenticCheckUpdateRespose> {
		try {
			const response = await fetch('https://api.github.com/repos/agenticeditor/binaries/releases/latest');
			...
			if (explicit) {
				if (response.ok) {
					if (!isUpToDate) {
						message = 'A new version of Agentic is available! Please reinstall (auto-updates are disabled on this OS) - it only takes a second!'
						action = 'reinstall'
					}
					else {
						message = 'Agentic is up-to-date!'
					}
				}
				else {
