...
import { AGENTIC_VIEW_CONTAINER_ID, AGENTIC_VIEW_ID } from './sidebarPane.js';
import { IMetricsService } from '../common/metricsService.js';
import { ICommandService } from '../../../../platform/commands/common/commands.js';
import { AGENTIC_TOGGLE_SETTINGS_ACTION_ID } from './agenticSettingsPane.js';
...
const AGENTIC_OPEN_SIDEBAR_ACTION_ID = 'agentic.sidebar.open'
registerAction2(class extends Action2 {
	constructor() {
		super({ id: AGENTIC_OPEN_SIDEBAR_ACTION_ID, title: localize2('agenticOpenSidebar', 'Agentic: Open Sidebar'), f1: true });
	}
	async run(accessor: ServicesAccessor): Promise<void> {
		const viewsService = accessor.get(IViewsService)
		const chatThreadsService = accessor.get(IChatThreadService)
		viewsService.openViewContainer(AGENTIC_VIEW_CONTAINER_ID)
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: AGENTIC_CTRL_L_ACTION_ID,
			f1: true,
			title: localize2('agenticCmdL', 'Agentic: Add Selection to Chat'),
			keybinding: {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		// open panel
		const wasAlreadyOpen = viewsService.isViewContainerVisible(AGENTIC_VIEW_CONTAINER_ID)
		if (!wasAlreadyOpen) {
			await commandService.executeCommand(AGENTIC_OPEN_SIDEBAR_ACTION_ID)
		}
...
const AGENTIC_CMD_SHIFT_L_ACTION_ID = 'agentic.cmdShiftL'
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: AGENTIC_CMD_SHIFT_L_ACTION_ID,
			title: 'New Chat',
			keybinding: {
			...
			icon: { id: 'add' },
			menu: [{ id: MenuId.ViewTitle, group: 'navigation', when: ContextKeyExpr.equals('view', AGENTIC_VIEW_ID), }],
		});
	}
	async run(accessor: ServicesAccessor): Promise<void> {
...
registerAction2(class extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		commandService.executeCommand(AGENTIC_CMD_SHIFT_L_ACTION_ID)
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: 'agentic.settingsAction',
			title: `Agentic's Settings`,
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		const commandService = accessor.get(ICommandService)
		commandService.executeCommand(AGENTIC_TOGGLE_SETTINGS_ACTION_ID)
	}
})
