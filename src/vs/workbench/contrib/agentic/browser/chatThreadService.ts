...
import { FeatureName, ModelSelection, ModelSelectionOptions } from '../common/agenticSettingsTypes.js';
...
import { AgenticFileSnapshot } from '../common/editCodeServiceTypes.js';
...
export type ThreadType = {
	...
	state: {
		...
		/** Stable id for per-ticket chat threads (set when user selects or plans a ticket). */
		jiraIssueKey?: string
...
export interface IChatThreadService {
	...
	findThreadIdForJiraIssue: (issueKey: string) => string | undefined
	switchToJiraIssue: (issueKey: string) => string
	openJiraBrowseSession: () => void
...
export const IChatThreadService = createDecorator<IChatThreadService>('agenticChatThreadService');
class ChatThreadService extends Disposable implements IChatThreadService {
	...
	// should probably re-use code from agentic/src/vs/base/common/marshalling.ts instead. but this is simple enough
			...
			if (wf?.phase === 'executing' && wf.planMarkdown) {
				this._setThreadState(threadId, {
					jiraWorkflow: {
						...
						jiraFinalizeStatus: undefined,
						jiraFinalizeMessage: undefined,
					},
				})
			}
		}
	...
	private _runToolCall = async (
		...
		if (!opts.preapproved) { // skip this if pre-approved
			...
			if (approvalType) {
				const jiraPhase = this.state.allThreads[threadId]?.state.jiraWorkflow?.phase
	...
	private _getCheckpointInfo = (checkpointMessage: ChatMessage & { role: 'checkpoint' }, fsPath: string, opts: { includeUserModifiedChanges: boolean }) => {
		const agenticFileSnapshot = checkpointMessage.agenticFileSnapshotOfURI ? checkpointMessage.agenticFileSnapshotOfURI[fsPath] ?? null : null
		if (!opts.includeUserModifiedChanges) { return { agenticFileSnapshot, } }

		const userModifiedAgenticFileSnapshot = fsPath in checkpointMessage.userModifications.agenticFileSnapshotOfURI ? checkpointMessage.userModifications.agenticFileSnapshotOfURI[fsPath] ?? null : null
		return { agenticFileSnapshot: userModifiedAgenticFileSnapshot ?? agenticFileSnapshot, }
	}
	...
	private _computeNewCheckpointInfo({ threadId }: { threadId: string }) {
		...
		const agenticFileSnapshotOfURI: { [fsPath: string]: AgenticFileSnapshot | undefined } = {}
		...
		for (const fsPath in lastIdxOfURI ?? {}) {
			const { model } = this._agenticModelService.getModelFromFsPath(fsPath)
			if (!model) continue
			...
			if (!res) continue
			const { agenticFileSnapshot: oldAgenticFileSnapshot } = res
			...
			const agenticFileSnapshot = this._editCodeService.getAgenticFileSnapshot(URI.file(fsPath))
			if (oldAgenticFileSnapshot === agenticFileSnapshot) continue
			agenticFileSnapshotOfURI[fsPath] = agenticFileSnapshot
		}
		...
		return { agenticFileSnapshotOfURI }
	}


	private _addUserCheckpoint({ threadId }: { threadId: string }) {
		const { agenticFileSnapshotOfURI } = this._computeNewCheckpointInfo({ threadId }) ?? {}
		this._addCheckpoint(threadId, {
			role: 'checkpoint',
			type: 'user_edit',
			agenticFileSnapshotOfURI: agenticFileSnapshotOfURI ?? {},
			userModifications: { agenticFileSnapshotOfURI: {}, },
		})
	}
	...
	private _addToolEditCheckpoint({ threadId, uri, }: { threadId: string, uri: URI }) {
		...
		const diffAreasSnapshot = this._editCodeService.getAgenticFileSnapshot(uri)
		this._addCheckpoint(threadId, {
			role: 'checkpoint',
			type: 'tool_edit',
			agenticFileSnapshotOfURI: { [uri.fsPath]: diffAreasSnapshot },
			userModifications: { agenticFileSnapshotOfURI: {} },
		})
	}
	...
	private _getCheckpointsBetween({ threadId, loIdx, hiIdx }: { threadId: string, loIdx: number, hiIdx: number }) {
		...
		for (let i = loIdx; i <= hiIdx; i += 1) {
			...
			for (const fsPath in message.agenticFileSnapshotOfURI) { // do not include userModified.beforeStrOfURI here, jumping should not include those changes
	...
	private _addUserModificationsToCurrCheckpoint({ threadId }: { threadId: string }) {
		...
		this._editMessageInThread(threadId, checkpointIdx, {
			...checkpoint,
			userModifications: { agenticFileSnapshotOfURI: agenticFileSnapshotOfURI ?? {}, },
		})
	}
		...
		if (toIdx < fromIdx) {
			...
			for (const fsPath in lastIdxOfURI) {
				...
				for (const k of idxes()) {
					...
					if (!res) continue
					const { agenticFileSnapshot } = res
					if (!agenticFileSnapshot) continue
					this._editCodeService.restoreAgenticFileSnapshot(URI.file(fsPath), agenticFileSnapshot)
					break
				}
			}
		}
...
*/
	...
	generateCodespanLink: IChatThreadService['generateCodespanLink'] = async ({ codespanStr: _codespanStr, threadId }) => {
		...
		if (codespanType === 'function-or-class') {
			...
			for (const uri of prevUris) {
				...
				const modelRef = await this._agenticModelService.getModelSafe(uri)
	...
	openNewThread() {
		...
		for (const threadId in currentThreads) {
			...
			if (t.messages.length === 0 && !t.state.jiraIssueKey && !t.state.linkedJiraIssue) {
	...
	findThreadIdForJiraIssue(issueKey: string): string | undefined {
		...
		for (const threadId in this.state.allThreads) {
			const thread = this.state.allThreads[threadId]
			if (!thread) continue
			const threadKey = thread.state.jiraIssueKey ?? thread.state.linkedJiraIssue?.key
			if (threadKey?.toUpperCase() === key) return threadId
		}
		return undefined
	}

	switchToJiraIssue(issueKey: string): string {
		const existingId = this.findThreadIdForJiraIssue(issueKey)
		if (existingId) {
		...
		this._storeAllThreads(newThreads)
		this._setState({ allThreads: newThreads, currentThreadId: newThread.id })
		this._setThreadState(newThread.id, { jiraIssueKey: issueKey.trim().toUpperCase() })
		return newThread.id
	}
	...
	setLinkedJiraIssue = (threadId: string, issue: LinkedJiraIssue | undefined) => {
		...
		this._setThreadState(threadId, { linkedJiraIssue: issue, jiraIssueKey: issue.key })
	}
	...
	approveJiraPlanAndExecute = async (threadId: string) => {
		...
		// Stop any in-flight planning stream BEFORE entering executing / Jira E2E.
		// Otherwise addUserMessageAndStreamResponse → abortRunning sees phase=executing and rolls back.
		if (this.streamState[threadId]?.isRunning) {
			await this.abortRunning(threadId)
		}
	...
	private _jiraWorkflowEditCount(threadId: string): number {
		const thread = this.state.allThreads[threadId]
		let count = 0
		for (const m of thread?.messages ?? []) {
			if (m.role === 'tool' && m.type === 'success' && (m.name === 'edit_file' || m.name === 'rewrite_file')) {
				count++
			}
		}
		return count
	...
	private _lastAssistantMessageHasJiraCompleteMarker(threadId: string): boolean {
		const thread = this.state.allThreads[threadId]
		const assistants = (thread?.messages ?? []).filter(m => m.role === 'assistant')
		const last = assistants[assistants.length - 1]
		return !!last && last.role === 'assistant' && last.displayContent.includes(JIRA_WORKFLOW_COMPLETE_MARKER)
	}
	...
	private async _finalizeJiraWorkflow(threadId: string, commentMarkdown: string) {
		...
		if (wf.phase === 'executing' && this._jiraWorkflowEditCount(threadId) === 0) {
			const noEditsMsg = 'No files were modified. Use edit_file or rewrite_file to implement the plan, then send the completion marker again.'
			this._setThreadState(threadId, {
				jiraWorkflow: {
					...wf,
					jiraFinalizeStatus: 'error',
					jiraFinalizeMessage: noEditsMsg,
				},
			})
			this._notificationService.notify({
				severity: Severity.Warning,
				message: `${issue.key}: ${noEditsMsg}`,
			})
			return
		}
	...
	private async _maybeFinalizeJiraOnRunEnd(threadId: string) {
		...
		// Only finalize when the agent explicitly signaled completion (fallback if marker was on last tool turn)
		if (!this._lastAssistantMessageHasJiraCompleteMarker(threadId)) return
