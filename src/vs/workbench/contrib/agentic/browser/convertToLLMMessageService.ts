...
import { ChatMode, FeatureName, ModelSelection, ProviderName } from '../common/agenticSettingsTypes.js';
...
import { JIRA_EXECUTE_USER_PREFIX, JiraWorkflowPhase, LinkedJiraIssue } from '../common/jiraTypes.js';

export const EMPTY_MESSAGE = '(empty message)'

/** Drop planning-phase chat turns so the model is not steered by read-only instructions. */
const chatMessagesForJiraExecution = (chatMessages: ChatMessage[]): ChatMessage[] => {
	let execIdx = -1
	for (let i = chatMessages.length - 1; i >= 0; i--) {
		const m = chatMessages[i]
		if (m.role === 'user' && m.displayContent.startsWith(JIRA_EXECUTE_USER_PREFIX)) {
			execIdx = i
			break
		}
	}
	if (execIdx < 0) return chatMessages
	return chatMessages.slice(execIdx)
}
...
const prepareOpenAIOrAnthropicMessages = ({
	...
	if (aiInstructions) sysMsgParts.push(`GUIDELINES (from the user's .agenticrules file):\n${aiInstructions}`)
...
export interface IConvertToLLMMessageService {
	...
	prepareLLMChatMessages: (opts: { chatMessages: ChatMessage[], chatMode: ChatMode, modelSelection: ModelSelection | null, linkedJiraIssue?: LinkedJiraIssue, jiraWorkflowPhase?: JiraWorkflowPhase }) => Promise<{ messages: LLMChatMessage[], separateSystemMessage: string | undefined }>
...
class ConvertToLLMMessageService extends Disposable implements IConvertToLLMMessageService {
	...
	constructor(
		...
		@IAgenticSettingsService private readonly agenticSettingsService: IAgenticSettingsService,
		@IAgenticModelService private readonly agenticModelService: IAgenticModelService,
		@IMCPService private readonly mcpService: IMCPService,
	) {
		super()
	}

	// Read .agenticrules files from workspace folders
	private _getAgenticRulesFileContents(): string {
		try {
			const workspaceFolders = this.workspaceContextService.getWorkspace().folders;
			let agenticRules = '';
			for (const folder of workspaceFolders) {
				const uri = URI.joinPath(folder.uri, '.agenticrules')
				const { model } = this.agenticModelService.getModel(uri)
				if (!model) continue
				agenticRules += model.getValue(EndOfLinePreference.LF) + '\n\n';
			}
			return agenticRules.trim();
		}
		catch (e) {
			return ''
		}
	...
	// Get combined AI instructions from settings and .agenticrules files
	private _getCombinedAIInstructions(): string {
		const globalAIInstructions = this.agenticSettingsService.state.globalSettings.aiInstructions;
		const agenticRulesFileContent = this._getAgenticRulesFileContents();

		const ans: string[] = []
		if (globalAIInstructions) ans.push(globalAIInstructions)
		if (agenticRulesFileContent) ans.push(agenticRulesFileContent)
		return ans.join('\n\n')
	}


	// system message
	private _generateChatMessagesSystemMessage = async (chatMode: ChatMode, specialToolFormat: 'openai-style' | 'anthropic-style' | 'gemini-style' | undefined, linkedJiraIssue?: LinkedJiraIssue, jiraWorkflowPhase?: JiraWorkflowPhase) => {
		const workspaceFolders = this.workspaceContextService.getWorkspace().folders.map(f => f.uri.fsPath)

		const openedURIs = this.modelService.getModels().filter(m => m.isAttachedToEditor()).map(m => m.uri.fsPath) || [];
		const activeURI = this.editorService.activeEditor?.resource?.fsPath;

		const skipDirectoryTree = jiraWorkflowPhase === 'planning' || jiraWorkflowPhase === 'executing'
		const directoryStr = skipDirectoryTree
			? '(omitted)'
			: await this.directoryStrService.getAllDirectoriesStr({
		...
		const systemMessage = chat_systemMessage({ workspaceFolders, openedURIs, directoryStr, activeURI, persistentTerminalIDs, chatMode, mcpTools, includeXMLToolDefinitions, linkedJiraIssue, jiraWorkflowPhase })
	...
	prepareLLMSimpleMessages: IConvertToLLMMessageService['prepareLLMSimpleMessages'] = ({ simpleMessages, systemMessage, modelSelection, featureName }) => {
		...
		const { overridesOfModel } = this.agenticSettingsService.state
		...
		const modelSelectionOptions = this.agenticSettingsService.state.optionsOfModelSelection[featureName][modelSelection.providerName]?.[modelSelection.modelName]
	...
	prepareLLMChatMessages: IConvertToLLMMessageService['prepareLLMChatMessages'] = async ({ chatMessages, chatMode, modelSelection, linkedJiraIssue, jiraWorkflowPhase }) => {
		if (modelSelection === null) return { messages: [], separateSystemMessage: undefined }

		const effectiveChatMessages = jiraWorkflowPhase === 'executing'
			? chatMessagesForJiraExecution(chatMessages)
			: chatMessages
		...
		const { disableSystemMessage } = this.agenticSettingsService.state.globalSettings;
		const fullSystemMessage = await this._generateChatMessagesSystemMessage(chatMode, specialToolFormat, linkedJiraIssue, jiraWorkflowPhase)
		const systemMessage = disableSystemMessage ? '' : fullSystemMessage;

		const modelSelectionOptions = this.agenticSettingsService.state.optionsOfModelSelection['Chat'][modelSelection.providerName]?.[modelSelection.modelName]

		// Get combined AI instructions
		const aiInstructions = this._getCombinedAIInstructions();
		const isReasoningEnabled = getIsReasoningEnabledState('Chat', providerName, modelName, modelSelectionOptions, overridesOfModel)
		const reservedOutputTokenSpace = getReservedOutputTokenSpace(providerName, modelName, { isReasoningEnabled, overridesOfModel })
		const llmMessages = this._chatMessagesToSimpleMessages(effectiveChatMessages)
