...
import { agenticPrefixAndSuffix, ctrlKStream_userMessage, ctrlKStream_systemMessage, defaultQuickEditFimTags, rewriteCode_systemMessage, rewriteCode_userMessage, searchReplaceGivenDescription_systemMessage, searchReplaceGivenDescription_userMessage, tripleTick, } from '../common/prompt/prompts.js';
import { IAgenticCommandBarService } from './agenticCommandBarService.js';
import { IKeybindingService } from '../../../../platform/keybinding/common/keybinding.js';
import { AGENTIC_ACCEPT_DIFF_ACTION_ID, AGENTIC_REJECT_DIFF_ACTION_ID } from './actionIDs.js';
...
import { IAgenticSettingsService } from '../common/agenticSettingsService.js';
import { FeatureName } from '../common/agenticSettingsTypes.js';
import { IAgenticModelService } from '../common/agenticModelService.js';
import { deepClone } from '../../../../base/common/objects.js';
import { acceptBg, acceptBorder, buttonFontSize, buttonTextColor, rejectBg, rejectBorder } from '../common/helpers/colors.js';
import { DiffArea, Diff, CtrlKZone, AgenticFileSnapshot, DiffAreaSnapshotEntry, diffAreaSnapshotKeys, DiffZone, TrackingZone, ComputedDiff } from '../common/editCodeServiceTypes.js';
...
class EditCodeService extends Disposable implements IEditCodeService {
	...
	constructor(
		...
		@IAgenticSettingsService private readonly _settingsService: IAgenticSettingsService,
		// @IFileService private readonly _fileService: IFileService,
		@IAgenticModelService private readonly _agenticModelService: IAgenticModelService,
		...
		const initializeModel = async (model: ITextModel) => {
			...
			await this._agenticModelService.initializeModel(model.uri)
	...
	// 		message: `Agentic Error: ${e.message}`,
	// 		actions: {
	// 			secondary: [{
	// 				id: 'agentic.onerror.opensettings',
	// 				enabled: true,
	// 				label: `Open Agentic's settings`,
	// 				tooltip: '',
	// 				class: undefined,
	// 				run: () => { this._commandService.executeCommand(AGENTIC_OPEN_SETTINGS_ACTION_ID) }
	// 			}]
	// 		},
	// 		source: details ? `(Hold ${isMacintosh ? 'Option' : 'Alt'} to hover) - ${details}\n\nIf this persists, feel free to [report](https://github.com/agenticeditor/agentic/issues/new) it.` : undefined
	// 	})
	// }
	...
	private _addDiffAreaStylesToURI = (uri: URI) => {
		const { model } = this._agenticModelService.getModel(uri)
		...
		for (const diffareaid of this.diffAreasOfURI[uri.fsPath] || []) {
			...
			if (diffArea.type === 'DiffZone') {
				...
				if (diffArea._streamState.isStreaming) {
					// sweepLine ... sweepLine
					const fn1 = this._addLineDecoration(model, diffArea._streamState.line, diffArea._streamState.line, 'agentic-sweepIdxBG')
					// sweepLine+1 ... endLine
					const fn2 = diffArea._streamState.line + 1 <= diffArea.endLine ?
						this._addLineDecoration(model, diffArea._streamState.line + 1, diffArea.endLine, 'agentic-sweepBG')
						: null
				...
				const fn = this._addLineDecoration(model, diffArea.startLine, diffArea.endLine, 'agentic-highlightBG')
	...
	private _addDiffStylesToURI = (uri: URI, diff: Diff) => {
		...
		if (type !== 'deletion') {
			const fn = this._addLineDecoration(model, diff.startLine, diff.endLine, 'agentic-greenBG', {
		...
		if (type !== 'insertion') {
			const consistentZoneId = this._consistentItemService.addConsistentItemToURI({
				...
				fn: (editor) => {
					...
					const domNode = document.createElement('div');
					domNode.className = 'agentic-redBG'
		...
		if (diffZone.type === 'DiffZone' && !diffZone._streamState.isStreaming) {
			...
			const consistentWidgetId = this._consistentItemService.addConsistentItemToURI({
				...
				fn: (editor) => {
					...
					else { throw new Error('Agentic 1') }
	...
	private _getCurrentAgenticFileSnapshot = (uri: URI): AgenticFileSnapshot => {
		const { model } = this._agenticModelService.getModel(uri)
	...
	private _restoreAgenticFileSnapshot = async (uri: URI, snapshot: AgenticFileSnapshot) => {
	...
	private _addToHistory(uri: URI, opts?: { onWillUndo?: () => void }) {
		const beforeSnapshot: AgenticFileSnapshot = this._getCurrentAgenticFileSnapshot(uri)
		let afterSnapshot: AgenticFileSnapshot | null = null
		...
		const elt: IUndoRedoElement = {
			...
			undo: async () => { opts?.onWillUndo?.(); await this._restoreAgenticFileSnapshot(uri, beforeSnapshot) },
			redo: async () => { if (afterSnapshot) await this._restoreAgenticFileSnapshot(uri, afterSnapshot) }
		}
		this._undoRedoService.pushElement(elt)

		const onFinishEdit = async () => {
			afterSnapshot = this._getCurrentAgenticFileSnapshot(uri)
			await this._agenticModelService.saveModel(uri)
		}
		return { onFinishEdit }
	}


	public getAgenticFileSnapshot(uri: URI) {
		return this._getCurrentAgenticFileSnapshot(uri)
	}


	public restoreAgenticFileSnapshot(uri: URI, snapshot: AgenticFileSnapshot): void {
		this._restoreAgenticFileSnapshot(uri, snapshot)
	}
	...
	private _writeStreamedDiffZoneLLMText(uri: URI, originalCode: string, llmTextSoFar: string, deltaText: string, latestMutable: StreamLocationMutable) {
		...
		if (!lastDiff) {
			...
			if (lastDiff.type === 'insertion' || lastDiff.type === 'edit')
				...
				throw new Error(`Agentic: diff.type not recognized on: ${lastDiff}`)
		}
	...
	public async revealFileInEditor(uri: URI): Promise<void> {
		...
		await this._codeEditorService.openCodeEditor(
			{ resource: uri, options: { revealIfOpened: true, preserveFocus: false, pinned: true } },
			editor ?? null,
		)
	}

	private _shouldAutoAcceptLLMChanges(): boolean {
		return this._settingsService.state.globalSettings.autoAcceptLLMChanges
			|| this._settingsService.isAnyJiraE2EActive()
	}

	public async callBeforeApplyOrEdit(givenURI: URI | 'current') {
		const uri = this._uriOfGivenURI(givenURI)
		if (!uri) return
		await this._agenticModelService.initializeModel(uri)
		await this._agenticModelService.saveModel(uri) // save the URI
		await this.revealFileInEditor(uri)
	}
	...
	public instantlyApplySearchReplaceBlocks({ uri, searchReplaceBlocks }: { uri: URI, searchReplaceBlocks: string }) {
		...
		const onDone = () => {
			...
			// auto accept
			if (this._shouldAutoAcceptLLMChanges()) {
	...
	private _initializeWriteoverStream(opts: StartApplyingOpts): [DiffZone, Promise<void>] | undefined {
		...
		if (from === 'ClickApply') {
			...
			throw new Error(`Agentic: diff.type not recognized on: ${from}`)
		}
		...
		if (from === 'ClickApply') {
			...
			const { prefix, suffix } = agenticPrefixAndSuffix({ fullFileStr: originalFileCode, startLine, endLine })
			...
			while (shouldSendAnotherMessage) {
				...
				streamRequestIdRef.current = this._llmMessageService.sendLLMMessage({
					...
					onFinalMessage: async (params) => {
						...
						const blocks = extractSearchReplaceBlocks(fullText)
						if (blocks.length === 0) {
							this._notificationService.info(`Agentic: We ran Fast Apply, but the LLM didn't output any changes.`)
						}
						this._writeURIText(uri, originalFileCode, 'wholeFileRange', { shouldRealignDiffAreas: true })
	...
	public async acceptDiff({ diffid }: { diffid: number }) {
		...
		if (diff.type === 'deletion') {
			...
			throw new Error(`Agentic error: ${diff}.type not recognized`)
		}
...
class AcceptRejectInlineWidget extends Widget implements IOverlayWidget {
	...
	constructor(
		...
		@IAgenticCommandBarService private readonly _agenticCommandBarService: IAgenticCommandBarService,
		...
		const getAcceptRejectText = () => {
			const acceptKeybinding = this._keybindingService.lookupKeybinding(AGENTIC_ACCEPT_DIFF_ACTION_ID);
			const rejectKeybinding = this._keybindingService.lookupKeybinding(AGENTIC_REJECT_DIFF_ACTION_ID);
			...
			const commandBarStateAtUri = this._agenticCommandBarService.stateOfURI[uri.fsPath];
		...
		this._register(this._agenticCommandBarService.onDidChangeState(e => {
