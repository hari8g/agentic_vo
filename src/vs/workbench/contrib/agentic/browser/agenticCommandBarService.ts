...
import { mountAgenticCommandBar } from './react/out/agentic-editor-widgets-tsx/index.js'
...
import { AGENTIC_ACCEPT_DIFF_ACTION_ID, AGENTIC_REJECT_DIFF_ACTION_ID, AGENTIC_GOTO_NEXT_DIFF_ACTION_ID, AGENTIC_GOTO_PREV_DIFF_ACTION_ID, AGENTIC_GOTO_NEXT_URI_ACTION_ID, AGENTIC_GOTO_PREV_URI_ACTION_ID, AGENTIC_ACCEPT_FILE_ACTION_ID, AGENTIC_REJECT_FILE_ACTION_ID, AGENTIC_ACCEPT_ALL_DIFFS_ACTION_ID, AGENTIC_REJECT_ALL_DIFFS_ACTION_ID } from './actionIDs.js';
...
export interface IAgenticCommandBarService {
...
export const IAgenticCommandBarService = createDecorator<IAgenticCommandBarService>('AgenticCommandBarService');
...
export class AgenticCommandBarService extends Disposable implements IAgenticCommandBarService {
	_serviceBrand: undefined;

	static readonly ID: 'agentic.AgenticCommandBarService'
	...
	async goToURIIdx(idx: number | null): Promise<void> {
		...
		const { model } = await this._agenticModelService.getModelSafe(nextURI);
		if (!model) return;
...
registerSingleton(IAgenticCommandBarService, AgenticCommandBarService, InstantiationType.Delayed); // delayed is needed here :(


export type AgenticCommandBarProps = {
	uri: URI | null;
	editor: ICodeEditor;
}
...
class AcceptRejectAllFloatingWidget extends Widget implements IOverlayWidget {
	...
	constructor({ editor }: { editor: ICodeEditor, },
		...
		this.instantiationService.invokeFunction(accessor => {
			...
			const res = mountAgenticCommandBar(root, accessor, { uri, editor } satisfies AgenticCommandBarProps)
			if (!res) return
			this._register(toDisposable(() => res.dispose?.()))
			this._register(editor.onWillChangeModel((model) => {
				const uri = model.newModelUrl
				res.rerender({ uri, editor } satisfies AgenticCommandBarProps)
			}))
		})
	}
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: AGENTIC_ACCEPT_DIFF_ACTION_ID,
			f1: true,
			title: localize2('agenticAcceptDiffAction', 'Agentic: Accept Diff'),
			keybinding: {
				primary: KeyMod.CtrlCmd | KeyMod.Alt | KeyMod.Shift | KeyCode.Enter,
				mac: { primary: KeyMod.WinCtrl | KeyMod.Alt | KeyCode.Enter },
				weight: KeybindingWeight.AgenticExtension,
			}
		});
	}
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		const commandBarService = accessor.get(IAgenticCommandBarService);
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: AGENTIC_REJECT_DIFF_ACTION_ID,
			f1: true,
			title: localize2('agenticRejectDiffAction', 'Agentic: Reject Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: AGENTIC_GOTO_NEXT_DIFF_ACTION_ID,
			f1: true,
			title: localize2('agenticGoToNextDiffAction', 'Agentic: Go to Next Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: AGENTIC_GOTO_PREV_DIFF_ACTION_ID,
			f1: true,
			title: localize2('agenticGoToPrevDiffAction', 'Agentic: Go to Previous Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: AGENTIC_GOTO_NEXT_URI_ACTION_ID,
			f1: true,
			title: localize2('agenticGoToNextUriAction', 'Agentic: Go to Next File with Diffs'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: AGENTIC_GOTO_PREV_URI_ACTION_ID,
			f1: true,
			title: localize2('agenticGoToPrevUriAction', 'Agentic: Go to Previous File with Diffs'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: AGENTIC_ACCEPT_FILE_ACTION_ID,
			f1: true,
			title: localize2('agenticAcceptFileAction', 'Agentic: Accept All Diffs in Current File'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: AGENTIC_REJECT_FILE_ACTION_ID,
			f1: true,
			title: localize2('agenticRejectFileAction', 'Agentic: Reject All Diffs in Current File'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: AGENTIC_ACCEPT_ALL_DIFFS_ACTION_ID,
			f1: true,
			title: localize2('agenticAcceptAllDiffsAction', 'Agentic: Accept All Diffs in All Files'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: AGENTIC_REJECT_ALL_DIFFS_ACTION_ID,
			f1: true,
			title: localize2('agenticRejectAllDiffsAction', 'Agentic: Reject All Diffs in All Files'),
			keybinding: {
