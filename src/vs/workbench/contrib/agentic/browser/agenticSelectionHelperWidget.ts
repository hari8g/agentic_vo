...
import { mountAgenticSelectionHelper } from './react/out/agentic-editor-widgets-tsx/index.js';
...
export type AgenticSelectionHelperProps = {
...
export class SelectionHelperContribution extends Disposable implements IEditorContribution, IOverlayWidget {
	public static readonly ID = 'editor.contrib.agenticSelectionHelper';
	// react
	...
	constructor(
		...
		@IAgenticSettingsService private readonly _agenticSettingsService: IAgenticSettingsService
	) {
		super();
		...
		this._instantiationService.invokeFunction(accessor => {
			...
			const res = mountAgenticSelectionHelper(content, accessor);
			if (!res) return;
	...
	private _showHelperForSelection(selection: Selection): void {
		...
		// rerender
		const enabled = this._agenticSettingsService.state.globalSettings.showInlineSuggestions
		...
		if (enabled) {
			this._rerender({ rerenderKey: this._rerenderKey } satisfies AgenticSelectionHelperProps)
