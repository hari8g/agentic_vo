...
import { displayInfoOfProviderName, ProviderName, providerNames, localProviderNames, featureNames, FeatureName, isFeatureNameDisabled } from '../../../../common/agenticSettingsTypes.js';
import { ChatMarkdownRender } from '../markdown/ChatMarkdownRender.js';
import { OllamaSetupInstructions, OneClickSwitchButton, SettingsForProvider, ModelDump } from '../agentic-settings-tsx/Settings.js';
import { ColorScheme } from '../../../../../../../platform/theme/common/theme.js';
import ErrorBoundary from '../sidebar-tsx/ErrorBoundary.js';
import { isLinux } from '../../../../../../../base/common/platform.js';
import { FileAccess } from '../../../../../../../base/common/network.js';

const MPS_LOGO_DARK_URI = FileAccess.asBrowserUri('vs/workbench/contrib/agentic/browser/media/mps-logo-dark.png').toString(true);
const MPS_LOGO_LIGHT_URI = FileAccess.asBrowserUri('vs/workbench/contrib/agentic/browser/media/mps-logo-light.png').toString(true);
...
export const AgenticOnboarding = () => {
	...
	const isOnboardingComplete = agenticSettingsState.globalSettings.isOnboardingComplete || OVERRIDE_VALUE

	const isDark = useIsDark()

	return (
		<div className={`@@agentic-scope ${isDark ? 'dark' : ''}`}>
			<div
				className={`
					bg-agentic-bg-3 fixed top-0 right-0 bottom-0 left-0 width-full z-[99999]
...
const AgenticIcon = () => {
	...
	useEffect(() => {
		const updateTheme = () => {
			...
			if (divRef.current) {
				divRef.current.style.maxWidth = '100%'
				divRef.current.style.aspectRatio = '1 / 1'
				divRef.current.style.backgroundImage = `url("${isDark ? MPS_LOGO_DARK_URI : MPS_LOGO_LIGHT_URI}")`
				...
				divRef.current.style.opacity = '0.92'
	...
	return <div ref={divRef} className='@@agentic-agentic-icon' />
}
...
const AddProvidersPage = ({ pageIndex, setPageIndex }: { pageIndex: number, setPageIndex: (index: number) => void }) => {
	...
	return (<div className="flex flex-col md:flex-row w-full h-[80vh] gap-6 max-w-[900px] mx-auto relative">
		{/* Left Column */}
		<div className="md:w-1/4 w-full flex flex-col gap-6 p-6 border-none border-agentic-border-2 h-full overflow-y-auto">
			{/* Tab Selector */}
		...
		<div className="flex-1 flex flex-col items-center justify-start p-6 h-full overflow-y-auto">
			...
			<div className="w-full max-w-xl mt-4 mb-10">
				...
				<div className="text-sm opacity-80 text-agentic-fg-3 my-4 w-full">{descriptionOfTab[currentTab]}</div>
			</div>
			...
			{providerNamesOfTab[currentTab].map((providerName) => (
				<div key={providerName} className="w-full max-w-xl mb-10">
					<div className="text-xl mb-2">
						...
						{providerName === 'gemini' && (
							<span
								data-tooltip-id="agentic-tooltip-provider-info"
			...
			{(currentTab === 'Local' || currentTab === 'Cloud/Other') && (
				<div className="w-full max-w-xl mt-8 bg-agentic-bg-2/50 rounded-lg p-6 border border-agentic-border-4">
					<div className="flex items-center gap-2 mb-4">
						<div className="text-xl font-medium">Models</div>
					</div>

					{currentTab === 'Local' && (
						<div className="text-sm opacity-80 text-agentic-fg-3 my-4 w-full">Local models should be detected automatically. You can add custom models below.</div>
					)}
...
// 		title:
// 			div
// 				"How would you like to use Agentic?"
// 		content:
...
const PreviousButton = ({ onClick, ...props }: { onClick: () => void } & React.ButtonHTMLAttributes<HTMLButtonElement>) => {
	return (
		<button
			onClick={onClick}
			className="px-6 py-2 rounded text-agentic-fg-3 opacity-80 hover:brightness-115 duration-600 transition-all"
			{...props}
		>
			Back
		</button>
...
const OllamaDownloadOrRemoveModelButton = ({ modelName, isModelInstalled, sizeGb }: { modelName: string, isModelInstalled: boolean, sizeGb: number | false | 'not-known' }) => {
	...
	return <a
		...
		className="flex items-center justify-center text-agentic-fg-2 hover:text-agentic-fg-1"
	>
...
const AgenticOnboardingContent = () => {


	const accessor = useAccessor()
	const agenticSettingsService = accessor.get('IAgenticSettingsService')
	const agenticMetricsService = accessor.get('IMetricsService')
	...
	const didFillInProviderSettings = selectedProviderName && agenticSettingsState.settingsOfProvider[selectedProviderName]._didFillInProviderSettings
	const isApiKeyLongEnoughIfApiKeyExists = selectedProviderName && agenticSettingsState.settingsOfProvider[selectedProviderName].apiKey ? agenticSettingsState.settingsOfProvider[selectedProviderName].apiKey.length > 15 : true
	const isAtLeastOneModel = selectedProviderName && agenticSettingsState.settingsOfProvider[selectedProviderName].models.length >= 1
	...
	const lastPagePrevAndNextButtons = <div className="max-w-[600px] w-full mx-auto flex flex-col items-end">
		<div className="flex items-center gap-2">
			...
			<PrimaryActionButton
				onClick={() => {
					agenticSettingsService.setGlobalSetting('isOnboardingComplete', true);
					agenticMetricsService.capture('Completed Onboarding', { selectedProviderName, wantToUseOption })
				}}
				ringSize={agenticSettingsState.globalSettings.isOnboardingComplete ? 'screen' : undefined}
			>Enter Agentic</PrimaryActionButton>
		</div>
	</div>
	...
	const detailedDescOfWantToUseOption: { [wantToUseOption in WantToUseOption]: string } = {
		...
		private: "Private-hosted so your data never leaves your computer or network. [Email us](mailto:founders@agenticeditor.com) for help setting up at your company.",
	...
	useEffect(() => {
		if (!agenticSettingsState.globalSettings.isOnboardingComplete) {
			setPageIndex(0)
		}
	}, [setPageIndex, agenticSettingsState.globalSettings.isOnboardingComplete])
		...
		0: <OnboardingPageShell
			content={
				<div className='flex flex-col items-center gap-8'>
					<div className="text-5xl font-light text-center">Welcome to Agentic</div>

					{/* Slice of Agentic image */}
			...
			content={
				<div>
					...
					<div className="mt-8 text-center flex flex-col items-center gap-4 w-full max-w-md mx-auto">
						<h4 className="text-agentic-fg-3 mb-4">Transfer your settings from an existing editor?</h4>
