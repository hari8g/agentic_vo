...
import '../styles.css'
import { AGENTIC_CTRL_K_ACTION_ID, AGENTIC_CTRL_L_ACTION_ID } from '../../../actionIDs.js';
import { Circle, MoreVertical } from 'lucide-react';
import { useEffect, useState } from 'react';

import { AgenticSelectionHelperProps } from '../../../../../../contrib/agentic/browser/agenticSelectionHelperWidget.js';
import { AGENTIC_OPEN_SETTINGS_ACTION_ID } from '../../../agenticSettingsPane.js';


export const AgenticSelectionHelperMain = (props: AgenticSelectionHelperProps) => {
	...
	>
		<AgenticSelectionHelper {...props} />
	</div>
}



const AgenticSelectionHelper = ({ rerenderKey }: AgenticSelectionHelperProps) => {
	...
	const ctrlLKeybind = keybindingService.lookupKeybinding(AGENTIC_CTRL_L_ACTION_ID)
	const ctrlKKeybind = keybindingService.lookupKeybinding(AGENTIC_CTRL_K_ACTION_ID)

	const dividerHTML = <div className='w-[0.5px] bg-agentic-border-3'></div>
	...
	useEffect(() => {
		const disposable = commandService.onWillExecuteCommand(e => {
			if (e.commandId === AGENTIC_CTRL_L_ACTION_ID || e.commandId === AGENTIC_CTRL_K_ACTION_ID) {
	...
	const defaultHTML = <>
		{ctrlLKeybind &&
			<div
				...
				'
				onClick={() => {
					commandService.executeCommand(AGENTIC_CTRL_L_ACTION_ID)
					setClickState('clickedOption');
				}}
			>
		...
		{ctrlKKeybind &&
			<div
				...
				'
				onClick={() => {
					commandService.executeCommand(AGENTIC_CTRL_K_ACTION_ID)
					setClickState('clickedOption');
				}}
			>
	...
	const moreOptionsHTML = <>
		<div
			...
			'
			onClick={() => {
				commandService.executeCommand(AGENTIC_OPEN_SETTINGS_ACTION_ID);
				setClickState('clickedOption');
			}}
		>
			Disable Suggestions?
	...
	return <div className='
		...
		border border-agentic-border-3 bg-agentic-bg-2
