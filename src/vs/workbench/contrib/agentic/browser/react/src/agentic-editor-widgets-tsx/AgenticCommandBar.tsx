...
import { AgenticCommandBarProps } from '../../../agenticCommandBarService.js';
import { Check, EllipsisVertical, Menu, MoveDown, MoveLeft, MoveRight, MoveUp, X } from 'lucide-react';
import {
	AGENTIC_GOTO_NEXT_DIFF_ACTION_ID,
	AGENTIC_GOTO_PREV_DIFF_ACTION_ID,
	AGENTIC_GOTO_NEXT_URI_ACTION_ID,
	AGENTIC_GOTO_PREV_URI_ACTION_ID,
	AGENTIC_ACCEPT_FILE_ACTION_ID,
	AGENTIC_REJECT_FILE_ACTION_ID,
	AGENTIC_ACCEPT_ALL_DIFFS_ACTION_ID,
	AGENTIC_REJECT_ALL_DIFFS_ACTION_ID
} from '../../../actionIDs.js';

export const AgenticCommandBarMain = ({ uri, editor }: AgenticCommandBarProps) => {
	const isDark = useIsDark()

	return <div
		className={`@@agentic-scope ${isDark ? 'dark' : ''}`}
	>
		<AgenticCommandBar uri={uri} editor={editor} />
	</div>
}
...
export const AgenticCommandBar = ({ uri, editor }: AgenticCommandBarProps) => {
	...
	const agenticModelService = accessor.get('IAgenticModelService')
	...
	const _upKeybinding = keybindingService.lookupKeybinding(AGENTIC_GOTO_PREV_DIFF_ACTION_ID);
	const _downKeybinding = keybindingService.lookupKeybinding(AGENTIC_GOTO_NEXT_DIFF_ACTION_ID);
	const _leftKeybinding = keybindingService.lookupKeybinding(AGENTIC_GOTO_PREV_URI_ACTION_ID);
	const _rightKeybinding = keybindingService.lookupKeybinding(AGENTIC_GOTO_NEXT_URI_ACTION_ID);
	const _acceptFileKeybinding = keybindingService.lookupKeybinding(AGENTIC_ACCEPT_FILE_ACTION_ID);
	const _rejectFileKeybinding = keybindingService.lookupKeybinding(AGENTIC_REJECT_FILE_ACTION_ID);
	const _acceptAllKeybinding = keybindingService.lookupKeybinding(AGENTIC_ACCEPT_ALL_DIFFS_ACTION_ID);
	const _rejectAllKeybinding = keybindingService.lookupKeybinding(AGENTIC_REJECT_ALL_DIFFS_ACTION_ID);
	...
	if (currFileIdx === null) {
		return (
			<div className="pointer-events-auto">
				<div className="flex bg-agentic-bg-2 shadow-md border border-agentic-border-2 [&>*:first-child]:pl-3 [&>*:last-child]:pr-3 [&>*]:border-r [&>*]:border-agentic-border-2 [&>*:last-child]:border-r-0">
		...
		<div className="pointer-events-auto">
			...
			{showAcceptRejectAllButtons && showAcceptRejectAll && (
				<div className="flex justify-end mb-1">
					<div className="inline-flex bg-agentic-bg-2 rounded shadow-md border border-agentic-border-2 overflow-hidden">
						<div className="flex items-center [&>*]:border-r [&>*]:border-agentic-border-2 [&>*:last-child]:border-r-0">
			...
			<div className="flex items-center bg-agentic-bg-2 rounded shadow-md border border-agentic-border-2 [&>*:first-child]:pl-3 [&>*:last-child]:pr-3 [&>*]:px-3 [&>*]:border-r [&>*]:border-agentic-border-2 [&>*:last-child]:border-r-0">
