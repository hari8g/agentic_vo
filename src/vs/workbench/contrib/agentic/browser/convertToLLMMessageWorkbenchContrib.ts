...
class ConvertContribWorkbenchContribution extends Disposable implements IWorkbenchContribution {
	static readonly ID = 'workbench.contrib.agentic.convertcontrib'
	...
	constructor(
		...
		const initializeURI = (uri: URI) => {
			...
			const agenticRulesURI = URI.joinPath(uri, '.agenticrules')
			this.agenticModelService.initializeModel(agenticRulesURI)
		}
