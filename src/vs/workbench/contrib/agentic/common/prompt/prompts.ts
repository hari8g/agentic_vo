...
import { ChatMode } from '../agenticSettingsTypes.js';
...
${directoryStr}
	...
	if (mode === 'agent') {
		...
		if (linkedJiraIssue && jiraWorkflowPhase === 'planning') {
			...
			details.push(`CRITICAL: You are in the EXECUTION phase. Ignore any earlier user or assistant message that said "read-only" or discouraged editing — those applied only to planning.`)
			details.push(`You MUST apply code changes with edit_file or rewrite_file. Reading or searching files alone does not complete the ticket.`)
			details.push(`Do NOT send ${JIRA_WORKFLOW_COMPLETE_MARKER} until at least one file has been successfully edited.`)
...
export const agenticPrefixAndSuffix = ({ fullFileStr, startLine, endLine }: { fullFileStr: string, startLine: number, endLine: number }) => {
...
// export const aiRegex_computeReplacementsForFile_userMessage = async ({ searchClause, replaceClause, fileURI, agenticFileService }: { searchClause: string, replaceClause: string, fileURI: URI, agenticFileService: IAgenticFileService }) => {

// 	// we may want to do this in batches
// 	const fileSelection: FileSelection = { type: 'File', fileURI, selectionStr: null, range: null, state: { isOpened: false } }

// 	const file = await stringifyFileSelections([fileSelection], agenticFileService)
