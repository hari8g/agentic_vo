...
type AgenticModelType = {
	model: ITextModel | null;
	editorModel: IResolvedTextEditorModel | null;
};

export interface IAgenticModelService {
	readonly _serviceBrand: undefined;
	initializeModel(uri: URI): Promise<void>;
	getModel(uri: URI): AgenticModelType;
	getModelFromFsPath(fsPath: string): AgenticModelType;
	getModelSafe(uri: URI): Promise<AgenticModelType>;
	saveModel(uri: URI): Promise<void>;

}

export const IAgenticModelService = createDecorator<IAgenticModelService>('agenticModelService');

class AgenticModelService extends Disposable implements IAgenticModelService {
	_serviceBrand: undefined;
	static readonly ID = 'agenticModelService';
	...
	getModelSafe = async (uri: URI): Promise<AgenticModelType> => {
...
registerSingleton(IAgenticModelService, AgenticModelService, InstantiationType.Eager);
