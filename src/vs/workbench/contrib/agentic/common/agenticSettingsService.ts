...
import { AGENTIC_SETTINGS_STORAGE_KEY } from './storageKeys.js';
import { defaultSettingsOfProvider, FeatureName, ProviderName, ModelSelectionOfFeature, SettingsOfProvider, SettingName, providerNames, ModelSelection, modelSelectionsEqual, featureNames, AgenticStatefulModelInfo, GlobalSettings, GlobalSettingName, defaultGlobalSettings, ModelSelectionOptions, OptionsOfModelSelection, ChatMode, OverridesOfModel, defaultOverridesOfModel, MCPUserStateOfName as MCPUserStateOfName, MCPUserState } from './agenticSettingsTypes.js';
...
export type AgenticSettingsState = {
...
// type RealAgenticSettings = Exclude<keyof AgenticSettingsState, '_modelOptions'>
// type EventProp<T extends RealAgenticSettings = RealAgenticSettings> = T extends 'globalSettings' ? [T, keyof AgenticSettingsState[T]] : T | 'all'


export interface IAgenticSettingsService {
	readonly _serviceBrand: undefined;
	readonly state: AgenticSettingsState; // in order to play nicely with react, you should immutably change state
	...
	dangerousSetState(newState: AgenticSettingsState): Promise<void>;
...
const _modelsWithSwappedInNewModels = (options: { existingModels: AgenticStatefulModelInfo[], models: string[], type: 'autodetected' | 'default' }) => {
	const { existingModels, models, type } = options

	const existingModelsMap: Record<string, AgenticStatefulModelInfo> = {}
...
const _stateWithMergedDefaultModels = (state: AgenticSettingsState): AgenticSettingsState => {
...
const _validatedModelState = (state: Omit<AgenticSettingsState, '_modelOptions'>): AgenticSettingsState => {
	...
	} satisfies AgenticSettingsState
...
class AgenticSettingsService extends Disposable implements IAgenticSettingsService {
	...
	dangerousSetState = async (newState: AgenticSettingsState) => {
	...
	async readAndInitializeState() {
		let readS: AgenticSettingsState
		try {
	...
	private async _readState(): Promise<AgenticSettingsState> {
		const encryptedState = this._storageService.get(AGENTIC_SETTINGS_STORAGE_KEY, StorageScope.APPLICATION)
	...
	private async _storeState() {
		...
		this._storageService.store(AGENTIC_SETTINGS_STORAGE_KEY, encryptedState, StorageScope.APPLICATION, StorageTarget.USER);
	}
	...
	setGlobalSetting: SetGlobalSettingFn = async (settingName, newVal) => {
		const newState: AgenticSettingsState = {
			...this.state,
			globalSettings: {
	...
	toggleModelHidden(providerName: ProviderName, modelName: string) {
		...
		const newModels: AgenticStatefulModelInfo[] = [
